...
var deployDir = filepath.Join(deploy, icingaPassive)
...
func compile() {
	if *allPlatforms {
		...
		compileGoPackage(icingaPassive)
	}
}
...
func createDistro() {
	packageName := fmt.Sprintf("%s-%s-%s.%s", icingaPassive, getPluginVersion(), getGOOS(), getArch())
...
func compileGoPackage(packageName string) {
	runProcess("go", "build", "-o", getGaugeExecutablePath(icingaPassive))
}
...
func copyPluginFiles(destDir string) {
	...
	if getGOOS() == "windows" {
		files[filepath.Join(getBinDir(), icingaPassive+".exe")] = bin
	} else {
		files[filepath.Join(getBinDir(), icingaPassive)] = bin
	}
...
func installPlugin(installPrefix string) {
	...
	pluginInstallPath := filepath.Join(installPrefix, icingaPassive, getPluginVersion())
